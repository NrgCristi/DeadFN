...
(function () {
	...
	try {
		global.BotConfig = JSON.parse(fs.readFileSync('./config/DeadFN/config.json', 'utf8', function (err, data) {
	...
	if (!BotConfig) {
		fs.mkdirSync(`./config/DeadFN/`, { recursive: true });
		fs.writeFile(`./config/DeadFN/config.json`, JSON.stringify({
	...
	app.listen(port, () => {
		console.log(`DeadFN is listening on port ${port}!`);
	});
